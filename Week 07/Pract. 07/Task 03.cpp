...
int main()
{
	...
	if (isInputDataValid(N)) {
		...
		addElementsToArray(number1, N);

		char number2[MAX_SIZE] = {};
		addElementsToArray(number2, N);

		long num1 = fromKToDecimal(number1, N, K);
		long num2 = fromKToDecimal(number2, N, K);
		...
		char res[MAX_SIZE] = {};
		fromDecimalToK(res, resultNum, K, count);
		cout << "Result: ";
		printArray(res, count);
	}
	else {
		printError();
	}
