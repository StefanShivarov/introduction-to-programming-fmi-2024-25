...
int main()
{
	...
	fillArray(symbolsSecond, symbolsLength);
	inDecimal = convertKthToDec(symbols, symbolsLength, numeralSystem);
	inDecimalSecond = convertKthToDec(symbolsSecond, symbolsLength, numeralSystem);
	inDecimalResult = inDecimal + inDecimalSecond;
	convertDecToKth(resultSymbols, resultLength, inDecimalResult, numeralSystem, symbolsLength);
