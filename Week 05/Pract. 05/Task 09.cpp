...
const size_t MAX_SIZE_ARR_RESULT = 100;

bool isDataValid(size_t length);
void addElements(int arr[], size_t length);
void unionFunction(int arr1[], int arr2[], size_t length1, size_t length2,
...
int main() {
	...
	if (isDataValid(length1) && isDataValid(length2)) {
		...
		int arrayResult[MAX_SIZE_ARR_RESULT] = {};
		size_t indResult = 0;
		...
		unionFunction(array1, array2, length1, length2, arrayResult, indResult);
...
void unionFunction(int arr1[], int arr2[], size_t length1, size_t length2,
	...
	for (size_t k = 0;k < length1;k++) {
		if (isIdentical(arrResult, indResult, arr1[k])) {
			arrResult[indResult] = arr1[k];
			indResult++;
		}
	}
